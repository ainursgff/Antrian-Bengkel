...
class _LayananMgmtScreenState extends State<LayananMgmtScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<AdminProvider>(context, listen: false).loadLayanan();
    });
  }
      ...
      body: Consumer<AdminProvider>(
        builder: (context, provider, _) {
          ...
          if (provider.layanan.isEmpty) {
            return const AppEmptyState(icon: Icons.build_outlined, title: 'Belum Ada Layanan', subtitle: 'Tambahkan layanan baru.');
          }

          return RefreshIndicator(
            onRefresh: () => provider.loadLayanan(),
  ...
  void _showForm(BuildContext context, {LayananModel? layanan}) {
    ...
    final estCtrl = TextEditingController(text: layanan?.estimasiMenit.toString() ?? '30');
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (context.mounted) {
                r['success'] == true ? Helpers.showSuccess(context, 'Layanan dihapus') : Helpers.showSnackbar(context, r['error'] ?? 'Gagal', isError: true);
              }
            },
