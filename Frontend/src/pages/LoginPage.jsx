          ...
          {showForgot ? (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>Lupa Password?</h3>
                <p style={{ color: '#64748b', fontSize: '0.9rem', marginTop: 8 }}>Masukkan email Anda dan kami akan mengirimkan instruksi pemulihan.</p>
              </div>
              ...
              {forgotStatus === 'success' ? (
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#16a34a', padding: '16px', borderRadius: 12, textAlign: 'center', marginBottom: 20 }}>
                  <i className="fas fa-envelope-open-text" style={{ fontSize: '2.5rem', marginBottom: 12 }}></i>
                  <h5 style={{ fontWeight: 800, margin: 0 }}>Cek Email Anda!</h5>
                  <p style={{ fontSize: '0.85rem', marginTop: 8, marginBottom: 0 }}>Link reset password telah dikirim ke <strong>{forgotEmail}</strong></p>
                </div>
              ) : (
                <form onSubmit={handleForgot}>
                  ...
                  <button type="submit" disabled={forgotStatus === 'loading'}
                    ...
                    {forgotStatus === 'loading' ? <span className="spinner-border spinner-border-sm" role="status"></span> : <><i className="fas fa-paper-plane"></i> Kirim Link Reset</>}
                  </button>
                </form>
              )}
              
              <div style={{ textAlign: 'center', marginTop: 24 }}>
                <button type="button" onClick={() => { setShowForgot(false); setForgotStatus(''); setForgotEmail(''); }} style={{ background: 'none', border: 'none', color: '#64748b', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
