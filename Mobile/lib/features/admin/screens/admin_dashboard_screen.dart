                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Status Montir
                        Text('Status Montir', style: Theme.of(context).textTheme.headlineSmall),
                        SizedBox(height: 14),
                        if (provider.montirList.isEmpty)
                          const AppEmptyState(
                            icon: Icons.engineering_rounded,
                            title: 'Tidak Ada Montir',
                            subtitle: 'Belum ada montir yang terdaftar.',
                          )
                        else
                          SizedBox(
                            height: 110,
                            child: ListView.separated(
                              ...
                              itemCount: provider.montirList.length,
                              separatorBuilder: (context, index) => SizedBox(width: 12),
                              itemBuilder: (context, index) {
                                final montir = provider.montirList[index];
                                final isSibuk = montir.busy;
                                return Container(
                                  width: 140,
                                  padding: const EdgeInsets.all(12),
                                  decoration: BoxDecoration(
                                    color: isSibuk ? AppColors.statusDilayani.withValues(alpha: 0.1) : AppColors.success.withValues(alpha: 0.1),
                                    borderRadius: BorderRadius.circular(16),
                                    border: Border.all(color: isSibuk ? AppColors.statusDilayani.withValues(alpha: 0.3) : AppColors.success.withValues(alpha: 0.3)),
                                  ),
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Icon(Icons.engineering_rounded, color: isSibuk ? AppColors.statusDilayani : AppColors.success, size: 20),
                                          SizedBox(width: 8),
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                            decoration: BoxDecoration(
                                              color: isSibuk ? AppColors.statusDilayani : AppColors.success,
                                              borderRadius: BorderRadius.circular(6),
                                            ),
                                            child: Text(
                                              isSibuk ? 'SIBUK' : 'KOSONG',
                                      ...
                                      SizedBox(height: 4),
                                      if (isSibuk && montir.activeAntrianNomor != null)
                                        Text(
                                          'Mengerjakan: ${montir.activeAntrianNomor}',
                                          style: TextStyle(fontSize: 10, color: AppColors.statusDilayani, fontWeight: FontWeight.w600),
                                          maxLines: 1,
                                          overflow: TextOverflow.ellipsis,
                                        )
                                      else
