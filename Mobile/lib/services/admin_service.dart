...
class AdminService {
  ...
  // =================== KATEGORI KENDARAAN ===================
  Future<List<KategoriKendaraanModel>> fetchKategori() async {
    try {
      final res = await _dio.get('/kategori-kendaraan');
      if (res.statusCode == 200 && res.data is List) {
        return (res.data as List).map((e) => KategoriKendaraanModel.fromJson(e as Map<String, dynamic>)).toList();
      }
      return [];
    } catch (_) { return []; }
  }
  ...
  Future<Map<String, dynamic>> createKategori(Map<String, dynamic> data) async {
    try {
      final res = await _dio.post('/kategori-kendaraan', data: data);
  ...
  Future<Map<String, dynamic>> updateKategori(int id, Map<String, dynamic> data) async {
    try {
      final res = await _dio.put('/kategori-kendaraan/$id', data: data);
      return res.data as Map<String, dynamic>;
    } on DioException catch (e) {
      return {'success': false, 'error': _extractError(e)};
    }
  ...
  Future<Map<String, dynamic>> deleteKategori(int id) async {
    try {
      final res = await _dio.delete('/kategori-kendaraan/$id');
      return res.data as Map<String, dynamic>;
    } on DioException catch (e) {
      return {'success': false, 'error': _extractError(e)};
    }
